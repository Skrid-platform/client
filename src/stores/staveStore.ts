...
export const useStaveStore = defineStore('stave', {
  ...
  actions: {
    ...
    changeLastNoteRhythm(newRhythm: string): void {
      ...
      // Remove last note
      const last_note: StaveNote = this.melody[this.melody.length - 1] as StaveNote;
      let note = last_note.keys[0];
      if (last_note.getNoteType() == 'r') note = 'r';
    ...
    clear_all_pattern(): void {
      ...
      // Cancel the previous pentagram
      const pentagram_svg = document.querySelector('#music-score svg')!;
